...
//import { isDateOccupied, getSeason, calculateTotalPrice } from '../utils/helpers';
...
const SearchForm = () => {
  ...
  try {
    ...
    const formattedData = data.map(hotel => ({
      id: hotel.id,
      name: hotel.tipo.charAt(0).toUpperCase() + hotel.tipo.slice(1),
      location: location,
      roomType: roomType,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {searchResults.map(hotel => (
                <div key={hotel.id} className="bg-white rounded-2xl shadow-xl p-6 transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                  <h4 className="text-2xl font-bold text-gray-900 mb-2">{hotel.name}</h4>
                  <p className="text-gray-600 mb-2">{hotel.location}</p>
                  <p className="text-gray-700 mb-1">Tipo de Habitación: <span className="font-semibold">{roomType.charAt(0).toUpperCase() + roomType.slice(1)}</span></p>
                  <p className="text-gray-700 mb-1">Personas: <span className="font-semibold">{numPeople}</span></p>
                  ...
                  <button
                    onClick={() => alert(`¡Reserva para ${hotel.name} confirmada! (Simulado)`)}
